...
Server::Server(const std::string& port, const std::string& pwd)
{
	...
	this->pwd = pwd;
	initServer(port);
	_commands = new Commands(clients, channels);
}
...
void Server::handleClientData(size_t i)
{
	...
	while ((n = read(fds[i].fd, buffer, sizeof(buffer) - 1)) > 0)
	{
		...
		{
			if (!handleClientMessage(client, message))
			{
				removeClient(i--, false);
				return; // Client removed, exit loop for this client
			}
...
void Server::removeClient(size_t i, bool graceful)
{
	...
	if (!graceful)
	{
		...
		_commands->executeCommand("QUIT :Connection closed", client);
	}
...
bool Server::handleClientMessage(Client& client, std::string& msg)
{
	...
	while ((pos = buffer.find("\r\n")) != std::string::npos)
	{
		...
		if (!_commands->executeCommand(line, client))
		{
			removeClient(std::distance(fds.begin(), std::find_if(fds.begin(), fds.end(), PollfdFinder(client.getFd()))), true);
			return false;
		}
	}
	return true;
